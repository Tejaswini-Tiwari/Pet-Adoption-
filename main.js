// We can use await without synchronization because in html we defined typw="module"
...
const template = document.querySelector("#animals-card");
...
function filterPets(species) {
    const allPets = document.querySelectorAll(".animals-card")
    if(species == "all"){
        allPets.forEach(el => {
            el.style.display = ""
        })
